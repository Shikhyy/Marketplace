...
export interface PaymentMetadata {
    ...
    paymentParameter?: {
        minerOf?: string;
        contentId?: string;
        type?: 'platform_fee' | 'rental' | 'subscription';
    };
}
...
export function useX402() {
    ...
    const handlePayment = async (metadata: PaymentMetadata): Promise<string> => {
        ...
        try {
            ...
            const isPlatformFee = metadata.paymentParameter?.type === 'platform_fee';

            if (isPlatformFee) {
                console.log("[useX402] Simulating USDC Platform Fee Transfer...");
            ...
            } else if (isRental && metadata.paymentParameter?.contentId) {
                // RENTAL
