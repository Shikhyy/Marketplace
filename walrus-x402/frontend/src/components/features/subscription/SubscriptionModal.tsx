...
import { CHAIN_ID, USDC_SEPOLIA_ADDRESS } from '@/config/constants';
...
export default function SubscriptionModal({ isOpen, onClose, plan, onSuccess }: SubscriptionModalProps) {
    const { handlePayment, paymentState, resetPayment, error: paymentError } = useX402();
    ...
    const onSubscribe = async () => {
        try {
            ...
            const metadata = {
                ...
                tokenAddress: USDC_SEPOLIA_ADDRESS, // USDC
                amount: (Number(plan.price) * 1e6).toString(), // Convert USDC to smallest unit (e.g., 1e6 for 6 decimals)
            ...
            // 4. Optimistic Success delay (handled mostly by hook now, just wait for routing)
            setTimeout(() => {
    ...
    return (
        <Modal open={isOpen} onClose={onClose}>
            <div className="space-y-6 text-center">
                ...
                {(paymentState === 'preparing' || paymentState === 'signing' || paymentState === 'confirming' || paymentState === 'verifying') && (
                    <div className="flex flex-col items-center gap-3 py-4">
                        <Loader2 className="w-8 h-8 animate-spin text-cyan-400" />
                        <p className="text-slate-300 animate-pulse">
                            {(paymentState === 'preparing' || paymentState === 'signing') && 'Confirm in Wallet...'}
